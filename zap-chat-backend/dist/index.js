...
let allSockets = [];
function broadcastTypingStatus(room, excludeSocket) {
    const typingUsers = allSockets.filter((user) => user.room === room && user.isTyping && user.socket !== excludeSocket).length;
    const typingMessage = {
        type: "typing_status",
        payload: {
            count: typingUsers,
...
wss.on("connection", (socket) => {
    ...
    socket.on("message", (message) => {
        ...
        if (!parsedMessage) {
            ...
            socket.send(JSON.stringify({ type: "error", message: "Message type is requried" }));
        }
